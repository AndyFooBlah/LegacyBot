...
export interface UserProfile {
  ...
  familyIds: string[];
  /**
   * Assigned directly in the Firestore console (see README). When true, the
   * `onUserProfileWritten` Cloud Function mirrors the value into the user's
   * Firebase Auth token as an `isSuperadmin` custom claim so that UI and
   * Firestore rules can check it without a separate round-trip.
   */
  isSuperadmin?: boolean;
}

// ---------------------------------------------------------------------------
// Invitation codes (superadmin-managed, gate new-family creation)
// ---------------------------------------------------------------------------

/**
 * Firestore document at invitationCodes/{code}.
 *
 * The document ID IS the code string (e.g. "A3B7Q2"). Codes are generated by
 * superadmins via the `generateInvitationCode` callable and redeemed by
 * `redeemInvitationCode`. Codes are multi-use: any active code can be
 * redeemed by many different users, but each user may redeem a given code at
 * most once (enforced by the presence of a redemption doc keyed by uid).
 */
...
/** Firestore document at invitationCodes/{code}/redemptions/{uid}. */
export interface InvitationCodeRedemption {
  id?: string;
...
export interface FamilyMemberRecord {
  ...
  inviteId?: string; // ID of the invitation used to join (for security validation)
...
export interface TranscriptEntry {
  ...
  /** Speaker attribution — set when the AI identifies a non-primary speaker (#98). */
  speakerLabel?: string;   // inferred speaker name (e.g. "Arthur") — null means primary storyteller
  speakerUid?: string;     // Firebase UID if the speaker is a known family member
...
export interface MiscFact {
  ...
  speakerLabel?: string;     // Speaker who stated this fact, when not the primary storyteller (#98)
  createdAt: Timestamp;
}
...
export enum ConnectionStatus {
  ...
  CONNECTED = 'CONNECTED',
  COMPLETED = 'COMPLETED', // session ended intentionally (AI or user)
  ERROR = 'ERROR',
}
