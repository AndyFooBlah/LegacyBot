...
*`onSessionCompleted` (Firestore trigger)* — fires when `sessions/{id}.status` → `completed`. First it runs **offline transcript refinement** (awaited, so the steps below consume the refined text), then in parallel runs the admin email and gap-analysis/embedding:

0. **Offline transcript refinement** (`functions/src/refineTranscript.ts` + `transcriptAlignment.ts`, #123): downloads the session's mixed audio and sends it to Gemini 3.1 Pro (audio input via the Files API + structured output) for a higher-fidelity, diarized transcript. Real-time Gemini Live transcription is tuned for latency, not accuracy, and every downstream artifact inherits its errors — this one-shot pass fixes mis-heard proper names and fuzzy boundaries. Refined utterances are aligned onto the existing `transcript/entries` array by time window **relative to the first content entry** (robust to any audio-vs-session-doc start offset), keeping entry boundaries so positional references (events' `sourceEntryIds`) stay valid; each replaced entry's prior text is preserved in `editHistory`/`originalText` tagged with the `ai-refinement` sentinel. Best-effort and idempotent: it sets `sessions/{id}.refinedAt` (and writes `sessions/{id}/analysis/refinement` metadata), skips sessions already refined or human-edited (a populated `editedEntries`), and is bounded by a **per-dossier monthly cap** (`reserveMonthlyRefinement`, `_usage/refinement/monthly/*`). Any failure is logged and swallowed — the original transcript is never lost.
1. Admin notification email (opted-in admins).
2. **Deep gap analysis** (`functions/src/analysis.ts`): reads all transcripts, events, and questions across every session for the dossier. Identifies timeline gaps, theme gaps, and implied-but-unexplored threads. Writes results to `dossiers/{id}/analysis/gapAnalysis`. Because refinement ran first, gap analysis and the transcript embedding both operate on the refined text.
3. **Transcript embedding** for semantic search: reads the `transcript/entries` array (skipping tool turns), chunks it, and writes vector embeddings. (Previously this read the transcript as a per-turn *collection* with `orderBy('timestamp')` — which matched zero docs, so embeddings were silently empty; fixed alongside #123.)
